...
{
    const float FULL_LINE_WIDTH = 0.625;
...
float rectAlpha(float2 texcoord, float2 scale, float fill, float useArrow)
{
    ...
    const float DOTTING_WIDTH = 0.25;
    ...
    return saturate(fillBox + frameBox);
}

void GetIndicatorAlpha_float(float2 texcoords, float shape, float fill, float useArrow, float coneRadius, out float alpha)
{
    float4x4 modelMat = UNITY_MATRIX_M;
    float2 scale = float2(length(modelMat[0]), length(modelMat[2]));
